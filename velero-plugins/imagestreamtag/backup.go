...
func (p *BackupPlugin) Execute(item runtime.Unstructured, backup *v1.Backup) (runtime.Unstructured, []velero.ResourceIdentifier, error) {
	...
	if referenceTag {
		...
		if imageStreamTag.Tag.From.Kind == "ImageStreamTag" {
			...
			_, err = client.ImageStreamTags(tagNamespace).Get(context.TODO(), imageStreamTag.Tag.From.Name, metav1.GetOptions{})
			if err == nil {
			...
			if len(tagNameSplit) == 2 && len(tagNameSplit[1]) > 0 {
				istagList, err := client.ImageStreamTags(tagNamespace).List(context.TODO(), metav1.ListOptions{})
				if err == nil {
